...
const SunriseAndSunset = (props: Props) => {
  const {cityName, weatherData, activeCardNumber} = useWeatherStore()
  if(!weatherData) return <Loader/>

  const sunrise = new Date(weatherData.city.sunrise * 1000).toLocaleString().slice(12 , 17)
  const sunset = new Date(weatherData.city.sunset * 1000).toLocaleString().slice(12 , 17)
